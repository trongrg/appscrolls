...
module AppScrolls
  class Scroll
    ...
    def self.run_after?(another)
      self.run_after.include?(another.key) || another.run_before.include?(self.key)
    end

    def self.run_before?(another)
      self.run_before.include?(another.key) || another.run_after.include?(self.key)
    end
